#!/usr/bin/env python3
"""Generate reusable, auditable shift annotations for Shiv article records."""
...
def fetch_article_rows(cur: sqlite3.Cursor) -> list[sqlite3.Row]:
    return cur.execute(
        """
        SELECT
            a.id,
...
def fetch_tags_by_article(cur: sqlite3.Cursor) -> dict[int, dict[str, set[str]]]:
    rows = cur.execute(
        """
        SELECT
            at.article_id,
    ...
    tags_by_article: dict[int, dict[str, set[str]]] = {}
    for row in rows:
        article_id = int(row["article_id"])
        bucket = tags_by_article.setdefault(article_id, {"labels": set(), "slugs": set()})
...
def main() -> int:
    ...
    cur = conn.cursor()
    ensure_annotation_tables(cur)

    article_rows = fetch_article_rows(cur)
    tags_by_article = fetch_tags_by_article(cur)
    ...
    if not args.dry_run:
        cur.execute(
            ...
            (
                ...
                "Deterministic annotation generation from title + summary + tags.",
            ),
        )
    ...
    for article in article_rows:
        ...
        tag_payload = tags_by_article.get(article_id, {"labels": set(), "slugs": set()})
        ...
        for shift_id in shift_scope:
            ...
            if args.dry_run:
                row = cur.execute(
                    ...
                    LIMIT 1
                    """,
                    (article_id, shift_id, args.annotation_version, fingerprint),
                ).fetchone()
                if row:
                    skipped += 1
